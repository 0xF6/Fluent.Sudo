namespace Fluent.sudo.Platforms
{
    ...
    public class MacOS : ICommandExecuter
    {
        private readonly ILogger<MacOS> log;
        public MacOS() => log = Log.Get<MacOS>();
        ...
        public async Task<ExecuteResult> Execute(string cmd, TimeSpan timeoutWait)
        {
            ...
            var zip = Path.Combine(instanceID, "sudo.zip");
            ...
            File.WriteAllBytes($"{Path.Combine(instanceID, "sudo.zip")}", Convert.FromBase64String(APPLET));

            await Process.Start("/usr/bin/unzip", $"-o {zip.E('"')} -d {instanceID.E('"')}").WaitForExitAsync();


            var plist = new[] { "Contents", "Info.plist" }.Aggregate(instanceID, Path.Combine);

            const string key = "CFBundleName";
            const string value = "sudo.app Password Prompt"; // TODO get app title in osx

            await Process.Start("/usr/bin/defaults", $"write {plist.E('"')} {key.E('"')} {value.E('"')}").WaitForExitAsync();


            var cmdPath = new[] { "Contents", "MacOS", "sudo-prompt-command" }.Aggregate(instanceID, Path.Combine);


            File.WriteAllText(cmdPath, $"cd {cwd.E('"')}\n{cmd}");

            var proc = new Process();

            var binPath = new []{ "Contents", "MacOS", "applet" }.Aggregate(instanceID, Path.Combine);

            proc.StartInfo = new ProcessStartInfo($"./{Path.GetFileName(binPath)}") { WorkingDirectory = Path.GetFullPath(binPath) };


            cwd = new[] { "Contents", "MacOS" }.Aggregate(instanceID, Path.Combine);


            Expression<Func<string, string>> raw = 
                s => Path.Combine(cwd, s).AsFile().When(x => x.Exists, x => x.ReadAll());

            var exp = raw.Compile();

            var status = exp("status");
            var stderr = exp("stderr");
            var stdout = exp("stdout");
            ...
            {
                var status_code = int.Parse(status.Trim());
            ...
            cleanUp();
            return new ExecuteResult(result, stdout, stderr, status);
        }


        

        private static readonly string APPLET =
